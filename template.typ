...
#let template(
  ...
  // 设置正文和代码的字体
  set text(font: (fonts.en-font, fonts.zh-font), size: 12pt, lang: "en", region: "cn")
  ...
  show link: it => {
    ...
    if it.body.has("text") and it.body.text in author-names {
      ...
      // underline(stroke: (dash: "densely-dotted"), text(fill: rgb("#0645AD"), it)) 
...
header: context {
  ...
  box(width: 100%, height: 50%)[
    #align(right+top)[
      ...
      #v(9pt, weak: true)
      #box(inset: (bottom: 10pt), [
        #image("figures/biaoqing.png", width: 30%)
      ])
    ]
  ]
